...
export const updateSettingHandler = (win: BrowserWindow, ku16: ILockController) => {
  ipcMain.handle("set-setting", async (event, payload: IUpdateSetting) => {
    ...
    testPort.open(async (error) => {
      ...
      if (error && !error.message.trim().toLowerCase().includes("access denied")) {
        ...
        win.webContents.send("connection", { title: "Device Connection", message: "ไม่สามารถบันทึกข้อมูลการเชื่อมต่อได้", suggestion: "ตรวจสอบข้อมูล port อีกครั้ง" }); 
        logger({ user: "system", message: "port updates failed." });
        return;
      }  else {
        win.webContents.send("set-setting-res", payload);
        win.webContents.send("connection", { title: "Device Connection", message: "บันทึกข้อมูลการเชื่อมต่อสำเร็จ", suggestion: "สามารถกลับไปใช้งานได้อีกครั้ง" }); 
